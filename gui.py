...
def glow(color=ACCENT, blur=20, strength=0.5):
    ...
    c = QColor(color); c.setAlphaF(strength)
    s.setColor(c); s.setOffset(0, 0)
    return s
...
class NavButton(QPushButton):
    ...
    def _refresh(self):
        if self._active:
            ...
            color = TEXT_PRI; fw = "700"
        else:
            bg = "background:transparent;"
            bl = "border-left:3px solid transparent;"
            color = TEXT_SEC; fw = "400"
...
class StatCard(QFrame):
    def __init__(self, icon, title, value, color=ACCENT, parent=None):
        ...
        tl.setStyleSheet(f"color:{TEXT_SEC};font-size:10px;letter-spacing:2px;background:transparent;border:none;")
        self.vl = QLabel(str(value))
        self.vl.setStyleSheet(f"color:{color};font-size:28px;font-weight:700;font-family:'Consolas',monospace;background:transparent;border:none;")
        lay.addLayout(top); lay.addWidget(tl); lay.addWidget(self.vl)
...
class CameraPage(QWidget):
    ...
    def _build_ui(self):
        ...
        feed_frame = QFrame()
        feed_frame.setStyleSheet(f"background:#000;border:1px solid {ACCENT}44;border-radius:12px;")
        feed_frame.setGraphicsEffect(glow(ACCENT,30,0.25))
        fl = QVBoxLayout(feed_frame); fl.setContentsMargins(0,0,0,0)

        hdr = QFrame(); hdr.setFixedHeight(36)
        hdr.setStyleSheet(f"background:{BG_PANEL};border-radius:12px 12px 0 0;border-bottom:1px solid {BORDER};")
        hl = QHBoxLayout(hdr); hl.setContentsMargins(12,0,12,0)
        for clr in [RED, YELLOW, GREEN]:
            d = QLabel("●"); d.setStyleSheet(f"color:{clr};font-size:10px;background:transparent;border:none;")
            hl.addWidget(d)
        hl.addStretch()
        cl = QLabel("LIVE FEED  —  CAM #01")
        cl.setStyleSheet(f"color:{TEXT_SEC};font-size:11px;font-family:'Consolas',monospace;background:transparent;border:none;")
        hl.addWidget(cl); hl.addStretch()
        fl.addWidget(hdr)

        self._placeholder = QLabel("[ NO SIGNAL ]")
        self._placeholder.setAlignment(Qt.AlignCenter)
        self._placeholder.setMinimumHeight(340)
        self._placeholder.setStyleSheet(f"color:{TEXT_SEC};font-size:14px;font-family:'Consolas',monospace;letter-spacing:4px;background:#000;border:none;")
        self.video_label = QLabel()
        self.video_label.setAlignment(Qt.AlignCenter)
        self.video_label.setMinimumSize(480,340)
        self.video_label.setStyleSheet("background:#000;border:none;")
        self.video_label.hide()
        fl.addWidget(self._placeholder); fl.addWidget(self.video_label)
        left.addWidget(feed_frame, stretch=1)

        ctrl = QHBoxLayout(); ctrl.setSpacing(12)

        self.btn_start = QPushButton("▶  START CAMERA")
        self.btn_start.setCursor(Qt.PointingHandCursor); self.btn_start.setFixedHeight(44)
        ...
        self.btn_export.setCursor(Qt.PointingHandCursor); self.btn_export.setFixedHeight(44)
        ...
        ctrl.addWidget(self.btn_start, stretch=2); ctrl.addWidget(self.btn_export, stretch=1)
        left.addLayout(ctrl)
        ...
        # Identity card
        id_card = QFrame(); id_card.setStyleSheet(card_style()); id_card.setFixedHeight(200)
        idl = QVBoxLayout(id_card); idl.setContentsMargins(20,16,20,16); idl.setSpacing(8)
        tit = QLabel("IDENTITY")
        tit.setStyleSheet(f"color:{TEXT_SEC};font-size:10px;letter-spacing:3px;background:transparent;border:none;")
        self.lbl_name = QLabel("UNKNOWN")
        self.lbl_name.setStyleSheet(f"color:{ACCENT};font-size:26px;font-weight:700;font-family:'Consolas',monospace;background:transparent;border:none;")
        self.lbl_conf = QLabel("Confidence: —")
        self.lbl_conf.setStyleSheet(f"color:{TEXT_SEC};font-size:12px;font-family:'Consolas',monospace;background:transparent;border:none;")
        self.conf_bar = QProgressBar(); self.conf_bar.setRange(0,100); self.conf_bar.setValue(0)
        self.conf_bar.setFixedHeight(6); self.conf_bar.setTextVisible(False)
        ...
        self.status_badge.setStyleSheet(f"background:{TEXT_SEC}22;color:{TEXT_SEC};border:1px solid {TEXT_SEC}33;border-radius:4px;padding:2px 8px;font-size:11px;font-family:'Consolas',monospace;font-weight:600;")
        idl.addWidget(tit); idl.addWidget(self.lbl_name); idl.addWidget(self.lbl_conf)
        idl.addWidget(self.conf_bar); idl.addStretch(); idl.addWidget(self.status_badge)
        right.addWidget(id_card)

        
        thr_card = QFrame(); thr_card.setStyleSheet(card_style())
        tl = QVBoxLayout(thr_card); tl.setContentsMargins(20,16,20,16); tl.setSpacing(10)
        tt = QLabel("THREAT MONITOR")
        tt.setStyleSheet(f"color:{TEXT_SEC};font-size:10px;letter-spacing:3px;background:transparent;border:none;")
        self.threat_lbl = QLabel("● SYSTEM SECURE")
        self.threat_lbl.setStyleSheet(f"color:{GREEN};font-size:14px;font-weight:600;font-family:'Consolas',monospace;background:transparent;border:none;")
        self.attempt_lbl = QLabel("Failed attempts: 0 / 3")
        self.attempt_lbl.setStyleSheet(f"color:{TEXT_SEC};font-size:12px;font-family:'Consolas',monospace;background:transparent;border:none;")
        # Blacklist indicator
        self.banned_indicator = QLabel("⛔  BLACKLIST : ARMED")
        ...
        self.banned_indicator.setStyleSheet(f"color:{color_ind};font-size:11px;font-family:'Consolas',monospace;background:transparent;border:none;")
        tl.addWidget(tt); tl.addWidget(self.threat_lbl); tl.addWidget(self.attempt_lbl)
        tl.addWidget(self.banned_indicator)
        right.addWidget(thr_card)

        # Recent activity card
        act_card = QFrame(); act_card.setStyleSheet(card_style())
        al = QVBoxLayout(act_card); al.setContentsMargins(20,16,20,16); al.setSpacing(8)
        at = QLabel("RECENT ACTIVITY")
        at.setStyleSheet(f"color:{TEXT_SEC};font-size:10px;letter-spacing:3px;background:transparent;border:none;")
        al.addWidget(at)
        self.activity_items = []
        for _ in range(4):
            item = QLabel("—")
            item.setStyleSheet(f"color:{TEXT_SEC};font-size:11px;font-family:'Consolas',monospace;background:transparent;border:none;padding:4px 0;")
            al.addWidget(item); self.activity_items.append(item)
    ...
    def _update_frame(self):
        ...
        # Vérification blacklist EN PRIORITÉ 
        ...
        if is_banned:
            ...
            self.lbl_conf.setStyleSheet(f"color:{RED};font-size:12px;font-family:'Consolas',monospace;background:transparent;border:none;")
        ...
        # Remettre la barre de conf en cyan si on sort du mode banni
        self.conf_bar.setStyleSheet(f"""
            QProgressBar{{background:{BORDER};border-radius:3px;border:none;}}
            QProgressBar::chunk{{background:qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 {ACCENT2},stop:1 {ACCENT});border-radius:3px;}}
        """)
        ...
        if not success: self._fail_count += 1
        else:           self._fail_count = 0
        ...
        cv2.putText(frame, name, (20,40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, clr_cv, 2)
    ...
    def _do_export(self):
        try:
            export_logs()
            ts = datetime.datetime.now().strftime("%H:%M:%S")
            self._push_activity(f"{ts}  PDF exported", ACCENT)
        except Exception as e:
...
class LogsPage(QWidget):
    ...
    def _build_ui(self):
        ...
        tl = QVBoxLayout(tc); tl.setContentsMargins(20,16,20,16); tl.setSpacing(12)

        hdr = QHBoxLayout()
        t = QLabel("ACCESS LOGS")
        t.setStyleSheet(f"color:{TEXT_PRI};font-size:14px;font-weight:600;font-family:'Consolas',monospace;background:transparent;border:none;")
        self.btn_refresh = QPushButton("↻  REFRESH")
        self.btn_refresh.setCursor(Qt.PointingHandCursor); self.btn_refresh.setFixedSize(110,32)
        self.btn_refresh.setStyleSheet(f"""
            QPushButton{{background:transparent;color:{ACCENT};border:1px solid {ACCENT}55;border-radius:6px;font-family:'Consolas',monospace;font-size:12px;}}
            QPushButton:hover{{background:{ACCENT}22;}}
        """)
        self.btn_refresh.clicked.connect(self.load_data)
        hdr.addWidget(t); hdr.addStretch(); hdr.addWidget(self.btn_refresh)
        tl.addLayout(hdr)
        ...
        self.table.setStyleSheet(f"""
            QTableWidget{{background:transparent;color:{TEXT_PRI};font-family:'Consolas',monospace;font-size:12px;border:none;outline:none;}}
            ...
            QHeaderView::section{{background:{BG_PANEL};color:{TEXT_SEC};font-family:'Consolas',monospace;font-size:10px;letter-spacing:2px;padding:10px 12px;border:none;border-bottom:1px solid {BORDER};}}
            ...
            for r, row in enumerate(rows):
                for c, val in enumerate(row):
                    ...
                    if c == 2:
                        v = str(val).upper()
                        if "GRANT" in v:
                            item.setForeground(QColor(GREEN));  granted += 1
                        elif "BANNED" in v:
                            item.setForeground(QColor(ORANGE)); banned  += 1
                        elif "DENY" in v or "FAIL" in v:
                            item.setForeground(QColor(RED));    denied  += 1
...
class BlacklistPage(QWidget):
    ...
    def _build_ui(self):
        ...
        sr = QHBoxLayout(); sr.setSpacing(14)
        sr.addWidget(self.stat_total); sr.addWidget(self.stat_alerts); sr.addStretch()
        root.addLayout(sr)

        # Main card
        mc = QFrame(); mc.setStyleSheet(card_style())
        ml = QVBoxLayout(mc); ml.setContentsMargins(20,16,20,16); ml.setSpacing(12)

        hdr = QHBoxLayout()
        t = QLabel("BLACKLIST — VISAGES BANNIS")
        t.setStyleSheet(f"color:{RED};font-size:14px;font-weight:600;font-family:'Consolas',monospace;background:transparent;border:none;letter-spacing:1px;")

        self.btn_add = QPushButton("＋  AJOUTER BANNI")
        self.btn_add.setCursor(Qt.PointingHandCursor); self.btn_add.setFixedHeight(32)
        ...
        self.btn_refresh2.setCursor(Qt.PointingHandCursor); self.btn_refresh2.setFixedSize(110,32)
        ...
        hdr.addWidget(self.btn_add); hdr.addWidget(self.btn_refresh2)
        ml.addLayout(hdr)

        # Table des bannis (depuis la DB)
        self.table = QTableWidget()
        self.table.setColumnCount(4)
        ...
        # Info section
        info = QFrame(); info.setStyleSheet(card_style(f"border-color:{RED}33;"))
        il = QHBoxLayout(info); il.setContentsMargins(16,12,16,12)
        info_txt = QLabel(
            "ℹ  Ajoute les photos du visage banni dans  data/banned/<NOM>/  (ex: data/banned/Intrus1/1.jpg)  "
            "puis redémarre l'application pour que le modèle se réentraîne."
        )
        info_txt.setWordWrap(True)
        info_txt.setStyleSheet(f"color:{TEXT_SEC};font-size:11px;font-family:'Consolas',monospace;background:transparent;border:none;")
    ...
    def load_data(self):
        try:
            ...
            rows = cur.fetchall()
            # Compte aussi les alertes déclenchées dans les logs
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        lf = QFrame(); lf.setFixedHeight(80)
        lf.setStyleSheet(f"background:{BG_PANEL};border-bottom:1px solid {BORDER};")
        ll = QVBoxLayout(lf); ll.setContentsMargins(20,0,20,0)
        lt = QLabel("VisionLock")
        lt.setStyleSheet(f"color:{ACCENT};font-size:18px;font-weight:700;font-family:'Consolas',monospace;background:transparent;border:none;letter-spacing:1px;")
        lb = QLabel("AI SECURITY ")
        lb.setStyleSheet(f"color:{TEXT_SEC};font-size:9px;font-family:'Consolas',monospace;letter-spacing:3px;background:transparent;border:none;")
        ll.addStretch(); ll.addWidget(lt); ll.addWidget(lb); ll.addStretch()
        sbl.addWidget(lf); sbl.addSpacing(16)

        nl = QLabel("NAVIGATION")
        nl.setStyleSheet(f"color:{TEXT_SEC};font-size:9px;letter-spacing:3px;padding-left:20px;background:transparent;border:none;")
        sbl.addWidget(nl); sbl.addSpacing(6)
        ...
        # Bannière blacklist dans la sidebar
        if self.banned_names:
            bl_badge = QFrame()
            bl_badge.setStyleSheet(f"background:{RED}18;border:1px solid {RED}33;border-radius:6px;margin:8px 12px;")
            bbl = QVBoxLayout(bl_badge); bbl.setContentsMargins(10,8,10,8); bbl.setSpacing(2)
            bl_t = QLabel(f"⛔  {len(self.banned_names)} BANNI(S)")
            bl_t.setStyleSheet(f"color:{RED};font-size:10px;font-weight:700;font-family:'Consolas',monospace;background:transparent;border:none;")
            bl_s = QLabel("Surveillance active")
            bl_s.setStyleSheet(f"color:{RED}99;font-size:9px;font-family:'Consolas',monospace;background:transparent;border:none;")
            bbl.addWidget(bl_t); bbl.addWidget(bl_s)
            sbl.addWidget(bl_badge)

        # Status bar
        sf = QFrame(); sf.setFixedHeight(56)
        sf.setStyleSheet(f"border-top:1px solid {BORDER};background:{BG_PANEL};")
        stl = QHBoxLayout(sf); stl.setContentsMargins(20,0,20,0)
        dot = QLabel("●"); dot.setStyleSheet(f"color:{GREEN};font-size:10px;background:transparent;border:none;")
        sys_lbl = QLabel("SYSTEM ONLINE")
        sys_lbl.setStyleSheet(f"color:{TEXT_SEC};font-size:10px;font-family:'Consolas',monospace;background:transparent;border:none;")
        stl.addWidget(dot); stl.addWidget(sys_lbl); stl.addStretch()
        sbl.addWidget(sf)
        ml.addWidget(sb)

        #Content area 
        content = QFrame(); content.setStyleSheet(f"background:{BG_DEEP};")
        ...
        tl2 = QLabel(now)
        tl2.setStyleSheet(f"color:{TEXT_SEC};font-size:11px;font-family:'Consolas',monospace;background:transparent;border:none;")
        tbl.addWidget(self.page_title); tbl.addStretch(); tbl.addWidget(tl2)
        cl.addWidget(topbar)
    ...
    def _switch(self, idx):
        ...
        if idx == 1: self.logs_page.load_data()
        if idx == 2: self.blacklist_page.load_data()
